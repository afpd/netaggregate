...
std::string IPv4Net::net()
{
    return inttoipv4(first_address);
}

std::string IPv4Net::mask()
{
...
int main(int argc, char** argv) {
    if (argc > 1) {
        std::cout<<"Programm accepts IP networks/addresses\ne.g. 3ffe::2/120 10.10.0.1/16 3ffe::15 172.16.14.14\nas stdin and outputs aggregated/summarized list to stdout"<<std::endl;
        return -1;
    }
    ...
    for (std::string line; std::getline(std::cin, line);) {
        ...
        if (line.length()<2 || line.length()>43) {
            ...
            //std::cerr << "IPv6 " << line << std::endl;
            auto ip6 = IPv6Net(line);
            ipv6s.push_back(ip6);
        } else if ( line.find('.') != std::string::npos ) {
            //std::cerr << "IPv4 " << line << std::endl;
